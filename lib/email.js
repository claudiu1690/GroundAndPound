...
// Backend URL for click-once email links (verify-email, email-change confirm)
// that must hit a server route directly. Falls back to APP_URL minus the Vite
// port shift if not set, but you really should set it explicitly in prod.
...
function verifyEmailTemplate({ fighterName, verifyUrl }) {
    return {
        subject: "Confirm your Ground & Pound email",
        html: `
            <p>Hi ${escapeHtml(fighterName || "fighter")},</p>
            <p>Welcome to Ground &amp; Pound. Confirm your email so we can
            send you password resets and account notifications if you ever
            need them.</p>
            <p><a href="${escapeAttr(verifyUrl)}">${escapeAttr(verifyUrl)}</a></p>
            <p>This link expires in 24 hours. If it does, you can request a
            fresh one from the banner at the top of the app.</p>
            <p>If you didn't sign up, ignore this email — your address won't
            be added to anything.</p>
            <p>— The Octagon Gazette</p>
        `.trim(),
    };
}
...
/**
 * Ops notification for a submitted "Report a Bug". EVERY user-controlled value
 * (description, reporterEmail, pageUrl, userAgent) is escaped via escapeHtml.
 * pageUrl is rendered as escaped TEXT only — never an <a href> — so a hostile
 * URL can't become a clickable link in the ops inbox.
 */
function bugReportNotificationTemplate({
    category,
    description,
    reporterEmail,
    fighterId,
    ...
    const label = BUG_CATEGORY_LABELS[category] || "Other";
    const reporter = reporterEmail ? escapeHtml(reporterEmail) : "anonymous";
    const fighter  = fighterId ? escapeHtml(fighterId) : "—";
    const account  = accountId ? escapeHtml(accountId) : "—";
    const page     = pageUrl ? escapeHtml(pageUrl) : "—";
    const agent    = userAgent ? escapeHtml(userAgent) : "—";
    const when     = createdAt ? new Date(createdAt).toISOString() : new Date().toISOString();
    return {
        subject: `[Bug] ${label} — G&P bug report`,
        html: `
            <p><strong>New bug report — ${escapeHtml(label)}</strong></p>
            <ul>
                <li><strong>Category:</strong> ${escapeHtml(label)}</li>
                <li><strong>Reporter:</strong> ${reporter}</li>
                <li><strong>Fighter ID:</strong> ${fighter}</li>
                <li><strong>Account ID:</strong> ${account}</li>
                <li><strong>Page URL:</strong> ${page}</li>
                <li><strong>User agent:</strong> ${agent}</li>
                <li><strong>Submitted:</strong> ${escapeHtml(when)}</li>
            </ul>
            <p><strong>Description:</strong></p>
            <pre style="white-space:pre-wrap;word-wrap:break-word;">${escapeHtml(description)}</pre>
        `.trim(),
    };
}
