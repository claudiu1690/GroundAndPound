...
- **The table** is paginated (20 per page, "Load More") and ranks fighters by DP within the active filter. When the **All** filter is active (unified view), each row also carries a coloured **division badge** (Prospect → Champion, using the division colour) so tiers are legible in the merged list; the badge is hidden when a single division is selected (redundant — every row is that tier). Rows also carry contextual tags (Belt Holder, Rival, You, Protected, streak ×1.25, cross-weight class), highlight the belt holder and your own row, and a "Last Active" column hints at who is at risk of inactivity decay.
