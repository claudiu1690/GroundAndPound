...
/**
 * Fetches GET /pvp/ladder/:wc/:seasonId?page=&limit=
 * Silent-refetch: loading only on first load (no prior data).
 */
export function usePvpLadder(wc, seasonId, page = 1) {
  ...
  const fetch = useCallback(
    async ({ silent = false } = {}) => {
      ...
      setError(null);
      try {
        const res = await api.pvpLadder(wc, seasonId, page);
        setData(res);
      } catch (e) {
